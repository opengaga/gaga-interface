import { ServerResponse, Config } from './types'
import { RequestError, RequestInternetError, RequestTimeoutError } from './utils'
      ...
      fetch(combineURL(this.baseURL, paramStr ? url + '?' + paramStr : url), {
        ...
        .then((json: any) => {
          ...
          if (json && json.code !== 200) {
            reject(new RequestError(json.code, json.msg || 'unknown error'))
          } else {
            resolve(json)
          }
        })
        .catch(() => {
          clearTimeout(abortId)
          reject(new RequestInternetError())
        })
    })
  }
